 ...
 * <h2>Configuration</h2>
 * <p>
 * This connector is configured with the set of properties described in {@link KingbaseESConnectorConfig}.
 *
 * @author Horia Chiorean
 */
public class KingbaseESConnector extends RelationalBaseSourceConnector {

    private static final Logger LOGGER = LoggerFactory.getLogger(KingbaseESConnector.class);
    private Map<String, String> props;

    public KingbaseESConnector() {
    }
    ...
    public Class<? extends Task> taskClass() {
        return KingbaseESConnectorTask.class;
    }
    ...
    public ConfigDef config() {
        return KingbaseESConnectorConfig.configDef();
    }

    @Override
    protected void validateConnection(Map<String, ConfigValue> configValues, Configuration config) {
        final ConfigValue databaseValue = configValues.get(RelationalDatabaseConnectorConfig.DATABASE_NAME.name());
        final ConfigValue slotNameValue = configValues.get(KingbaseESConnectorConfig.SLOT_NAME.name());
        final ConfigValue pluginNameValue = configValues.get(KingbaseESConnectorConfig.PLUGIN_NAME.name());
        if (!databaseValue.errorMessages().isEmpty() || !slotNameValue.errorMessages().isEmpty()
                || !pluginNameValue.errorMessages().isEmpty()) {
            return;
        }

        final KingbaseESConnectorConfig postgresConfig = new KingbaseESConnectorConfig(config);
        final ConfigValue hostnameValue = configValues.get(RelationalDatabaseConnectorConfig.HOSTNAME.name());
        // Try to connect to the database ...
        try (KingbaseESConnection connection = new KingbaseESConnection(postgresConfig.getJdbcConfig())) {
            try {
    ...
    protected Map<String, ConfigValue> validateAllFields(Configuration config) {
        return config.validate(KingbaseESConnectorConfig.ALL_FIELDS);
    }
}
