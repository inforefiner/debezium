...
import io.debezium.connector.kingbase.KingbaseESConnectorConfig.HStoreHandlingMode;
import io.debezium.connector.kingbase.KingbaseESConnectorConfig.IntervalHandlingMode;
import io.debezium.connector.kingbase.data.Ltree;
 ...
 */
public class KingbaseESValueConverter extends JdbcValueConverters {
    ...
    public static KingbaseESValueConverter of(KingbaseESConnectorConfig connectorConfig, Charset databaseCharset, TypeRegistry typeRegistry) {
        return new KingbaseESValueConverter(
                databaseCharset,
    ...
    protected KingbaseESValueConverter(Charset databaseCharset, DecimalMode decimalMode,
    ...
    public SchemaBuilder schemaBuilder(Column column) {
        ...
        switch (oidValue) {
            ...
            case KbOid.JSONB_OID:
            case KbOid.JSON:
                return Json.builder();
            case KbOid.TSRANGE_OID:
            case KbOid.TSTZRANGE_OID:
            case KbOid.DATERANGE_OID:
            case KbOid.INET_OID:
            case KbOid.CIDR_OID:
            case KbOid.MACADDR_OID:
            case KbOid.MACADDR8_OID:
            case KbOid.INT4RANGE_OID:
            case KbOid.NUM_RANGE_OID:
            case KbOid.INT8RANGE_OID:
            ...
            default:
                ...
                final KingbaseESType resolvedType = typeRegistry.get(oidValue);
    ...
    private ValueConverter createArrayConverter(Column column, Field fieldDefn) {
        KingbaseESType arrayType = typeRegistry.get(column.nativeType());
        KingbaseESType elementType = arrayType.getElementType();
    ...
    protected Object convertGeometry(Column column, Field fieldDefn, Object data) {
        final KingbaseESGeometry empty = KingbaseESGeometry.createEmpty();
        return convertValue(column, fieldDefn, data, Geometry.createValue(fieldDefn.schema(), empty.getWkb(), empty.getSrid()), (r) -> {
            try {
                ...
                if (data instanceof byte[]) {
                    KingbaseESGeometry geom = KingbaseESGeometry.fromHexEwkb(new String((byte[]) data, "ASCII"));
                    r.deliver(Geometry.createValue(schema, geom.getWkb(), geom.getSrid()));
                }
                else if (data instanceof KBobject) {
                    KBobject pgo = (KBobject) data;
                    KingbaseESGeometry geom = KingbaseESGeometry.fromHexEwkb(pgo.getValue());
                    r.deliver(Geometry.createValue(schema, geom.getWkb(), geom.getSrid()));
                }
                else if (data instanceof String) {
                    KingbaseESGeometry geom = KingbaseESGeometry.fromHexEwkb((String) data);
                    r.deliver(Geometry.createValue(schema, geom.getWkb(), geom.getSrid()));
                }
            }
    ...
    protected Object convertPoint(Column column, Field fieldDefn, Object data) {
        return convertValue(column, fieldDefn, data, Point.createValue(fieldDefn.schema(), 0, 0), (r) -> {
            ...
            if (data instanceof KBpoint) {
                KBpoint pgPoint = (KBpoint) data;
                r.deliver(Point.createValue(schema, pgPoint.x, pgPoint.y));
            }
            else if (data instanceof String) {
                String dataString = data.toString();
                try {
                    KBpoint pgPoint = new KBpoint(dataString);
                    r.deliver(Point.createValue(schema, pgPoint.x, pgPoint.y));
                }
                catch (SQLException e) {
    ...
    protected Object convertArray(Column column, Field fieldDefn, KingbaseESType elementType, ValueConverter elementConverter, Object data) {
        return convertValue(column, fieldDefn, data, Collections.emptyList(), (r) -> {
    ...
    private Object resolveArrayValue(Object value, KingbaseESType elementType) {
