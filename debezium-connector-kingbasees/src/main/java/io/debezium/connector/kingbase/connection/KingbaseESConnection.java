...
import io.debezium.connector.kingbase.*;
 ...
 */
public class KingbaseESConnection extends JdbcConnection {

    private static Logger LOGGER = LoggerFactory.getLogger(KingbaseESConnection.class);
    ...
    protected static final ConnectionFactory FACTORY = JdbcConnection.patternBasedFactory(URL_PATTERN,
            ...
            KingbaseESConnection.class.getClassLoader(), JdbcConfiguration.PORT.withDefault(KingbaseESConnectorConfig.PORT.defaultValueAsString()));
    ...
    private final KingbaseESDefaultValueConverter defaultValueConverter;

    /**
     * Creates a Postgres connection using the supplied configuration.
     * If necessary this connection is able to resolve data type mappings.
     * Such a connection requires a {@link KingbaseESValueConverter}, and will provide its own {@link TypeRegistry}.
     * Usually only one such connection per connector is needed.
     *
     * @param config                {@link Configuration} instance, may not be null.
     * @param valueConverterBuilder supplies a configured {@link KingbaseESValueConverter} for a given {@link TypeRegistry}
     */
    public KingbaseESConnection(Configuration config, PostgresValueConverterBuilder valueConverterBuilder) {
        super(config, FACTORY, KingbaseESConnection::validateServerVersion, KingbaseESConnection::defaultSettings, "\"", "\"");
        ...
        if (Objects.isNull(valueConverterBuilder)) {
            ...
            final KingbaseESValueConverter valueConverter = valueConverterBuilder.build(this.typeRegistry);
            this.defaultValueConverter = new KingbaseESDefaultValueConverter(valueConverter, this.getTimestampUtils());
            // this.defaultValueConverter = new PostgresDefaultValueConverter(valueConverter, this.getTimestampUtils());
        }
    }

    /**
     * Create a Kingbase connection using the supplied configuration and {@link TypeRegistry}
     *
     * @param config       {@link Configuration} instance, may not be null.
     * @param typeRegistry an existing/already-primed {@link TypeRegistry} instance
     */
    public KingbaseESConnection(Configuration config, TypeRegistry typeRegistry) {
        super(config, FACTORY, KingbaseESConnection::validateServerVersion, KingbaseESConnection::defaultSettings, "\"", "\"");
        if (Objects.isNull(typeRegistry)) {
            ...
            final KingbaseESValueConverter valueConverter = KingbaseESValueConverter.of(new KingbaseESConnectorConfig(config), this.getDatabaseCharset(), typeRegistry);
            this.defaultValueConverter = new KingbaseESDefaultValueConverter(valueConverter, this.getTimestampUtils());
        }
    }

    /**
     * Creates a Kingbase connection using the supplied configuration.
     * The connector is the regular one without datatype resolution capabilities.
     *
     * @param config {@link Configuration} instance, may not be null.
     */
    public KingbaseESConnection(Configuration config) {
     ...
     * <p>
     * To fetch the slot without the retries, use the {@link KingbaseESConnection#fetchReplicationSlotInfo} call
     *
    ...
    private Optional<ColumnEditor> doReadTableColumn(ResultSet columnMetadata, TableId tableId, Tables.ColumnNameFilter columnFilter)
        ...
        if (columnFilter == null || columnFilter.matches(tableId.catalog(), tableId.schema(), tableId.table(), columnName)) {
            ...
            final KingbaseESType nativeType = getTypeRegistry().get(column.typeName());
    ...
    public <T extends DatabaseSchema<TableId>> Object getColumnValue(ResultSet rs, int columnIndex, Column column,
        ...
        try {
            ...
            final KingbaseESType type = ((KingbaseESSchema) schema).getTypeRegistry().get(columnTypeName);
            ...
            switch (type.getOid()) {
                case KbOid.MONEY:
                    ...
                    if (sMoney.startsWith("-")) {
                        ...
                        return new KBmoney(negativeMoney).val;
                    }
                    return new KBmoney(sMoney).val;
                case KbOid.BIT:
                ...
                case KbOid.NUMERIC:
                    ...
                    Optional<SpecialValueDecimal> value = KingbaseESValueConverter.toSpecialValue(s);
    ...
    public interface PostgresValueConverterBuilder {
        KingbaseESValueConverter build(TypeRegistry registry);
    }
}
