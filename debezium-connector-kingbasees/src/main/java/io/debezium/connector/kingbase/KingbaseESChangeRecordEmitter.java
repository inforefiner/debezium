 ...
 */
public class KingbaseESChangeRecordEmitter extends RelationalChangeRecordEmitter {
    ...
    public KingbaseESChangeRecordEmitter(Partition partition, OffsetContext offset, Clock clock, KingbaseESConnectorConfig connectorConfig, KingbaseESSchema schema,
                                         KingbaseESConnection connection, TableId tableId,
    ...
    static Optional<DataCollectionSchema> updateSchema(TableId tableId, ChangeRecordEmitter changeRecordEmitter) {
        return ((KingbaseESChangeRecordEmitter) changeRecordEmitter).newTable(tableId);
    }
    ...
    private Table tableFromFromMessage(List<ReplicationMessage.Column> columns, Table table) {
        final TableEditor combinedTable = table.edit()
                .setColumns(columns.stream()
                        .map(column -> {
                            final KingbaseESType type = column.getType();
