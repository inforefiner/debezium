...
import io.debezium.connector.kingbase.connection.*;
import io.debezium.connector.kingbase.connection.ReplicationMessage.Operation;
 ...
 */
public class KingbaseESStreamingChangeEventSource implements StreamingChangeEventSource<KingbaseESPartition, KingbaseESOffsetContext> {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(KingbaseESStreamingChangeEventSource.class);
    ...
    private final KingbaseESConnection connection;
    ...
    public KingbaseESStreamingChangeEventSource(KingbaseESConnectorConfig connectorConfig, Snapshotter snapshotter,
                                                KingbaseESConnection connection, EventDispatcher<TableId> dispatcher, ErrorHandler errorHandler, Clock clock,
                                                KingbaseESSchema schema, KingbaseESTaskContext taskContext, ReplicationConnection replicationConnection) {
    ...
    @Override
    public void execute(ChangeEventSourceContext context, KingbaseESPartition partition, KingbaseESOffsetContext offsetContext)
        ...
        if (!hasStartLsnStoredInContext) {
            offsetContext = KingbaseESOffsetContext.initialContext(connectorConfig, connection, clock);
        }
        ...
        try {
            ...
            stream.startKeepAlive(Threads.newSingleThreadExecutor(KingbaseESConnector.class, connectorConfig.getLogicalName(), KEEP_ALIVE_THREAD_NAME));
    ...
    private void processMessages(ChangeEventSourceContext context, KingbaseESPartition partition, KingbaseESOffsetContext offsetContext, final ReplicationStream stream)
            ...
            boolean receivedMessage = stream.readPending(message -> {
                ...
                // DML event
                    ...
                    if (message.getOperation() != Operation.NOOP) {
                        tableId = KingbaseESSchema.parse(message.getTable());
                    ...
                    boolean dispatched = message.getOperation() != Operation.NOOP && dispatcher.dispatchDataChangeEvent(
                            tableId,
                            new KingbaseESChangeRecordEmitter(
                                    partition,
                                    offsetContext,
                                    clock,
                                    connectorConfig,
    ...
    private void commitMessage(KingbaseESPartition partition, KingbaseESOffsetContext offsetContext, final Lsn lsn) throws SQLException, InterruptedException {
        lastCompletelyProcessedLsn = lsn;
    ...
    public void commitOffset(Map<String, ?> offset) {
        try {
            ...
            final Lsn commitLsn = Lsn.valueOf((Long) offset.get(KingbaseESOffsetContext.LAST_COMMIT_LSN_KEY));
            final Lsn changeLsn = Lsn.valueOf((Long) offset.get(KingbaseESOffsetContext.LAST_COMPLETELY_PROCESSED_LSN_KEY));
     ...
     */
    private boolean isInPreSnapshotCatchUpStreaming(KingbaseESOffsetContext offsetContext) {
