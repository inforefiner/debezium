...
import io.debezium.connector.kingbase.connection.KingbaseESConnection.PostgresValueConverterBuilder;
 ...
 */
public class KingbaseESConnectorTask extends BaseSourceTask<KingbaseESPartition, KingbaseESOffsetContext> {

    private static final Logger LOGGER = LoggerFactory.getLogger(KingbaseESConnectorTask.class);
    private static final String CONTEXT_NAME = "kingbase-connector-task";

    private volatile KingbaseESTaskContext taskContext;
    private volatile ChangeEventQueue<DataChangeEvent> queue;
    private volatile KingbaseESConnection jdbcConnection;
    private volatile KingbaseESConnection heartbeatConnection;
    private volatile KingbaseESSchema schema;

    @Override
    public ChangeEventSourceCoordinator<KingbaseESPartition, KingbaseESOffsetContext> start(Configuration config) {
        final KingbaseESConnectorConfig connectorConfig = new KingbaseESConnectorConfig(config);
        final TopicSelector<TableId> topicSelector = KingbaseESTopicSelector.create(connectorConfig);
        ...
        heartbeatConnection = new KingbaseESConnection(connectorConfig.getJdbcConfig());
        final Charset databaseCharset = heartbeatConnection.getDatabaseCharset();

        final PostgresValueConverterBuilder valueConverterBuilder = (typeRegistry) -> KingbaseESValueConverter.of(
                connectorConfig,
                databaseCharset,
                typeRegistry);
        ...
        jdbcConnection = new KingbaseESConnection(connectorConfig.getJdbcConfig(), valueConverterBuilder);
        try {
        ...
        schema = new KingbaseESSchema(connectorConfig, typeRegistry, topicSelector, valueConverterBuilder.build(typeRegistry));
        this.taskContext = new KingbaseESTaskContext(connectorConfig, schema, topicSelector);
        final Offsets<KingbaseESPartition, KingbaseESOffsetContext> previousOffsets = getPreviousOffsets(
                new KingbaseESPartition.Provider(connectorConfig), new KingbaseESOffsetContext.Loader(connectorConfig));
        final Clock clock = Clock.system();
        final KingbaseESOffsetContext previousOffset = previousOffsets.getTheOnlyOffset();
        ...
        try {
            ...
            ErrorHandler errorHandler = new KingbaseESErrorHandler(connectorConfig.getLogicalName(), queue);

            final KingbaseESEventMetadataProvider metadataProvider = new KingbaseESEventMetadataProvider();
            ...
            final EventDispatcher<TableId> dispatcher = new EventDispatcher<>(
                    ...
                    DataChangeEvent::new,
                    KingbaseESChangeRecordEmitter::updateSchema,
                    metadataProvider,
                    heartbeat,
                    schemaNameAdjuster,
                    jdbcConnection);

            ChangeEventSourceCoordinator<KingbaseESPartition, KingbaseESOffsetContext> coordinator = new KingbaseESChangeEventSourceCoordinator(
                    previousOffsets,
                    errorHandler,
                    KingbaseESConnector.class,
                    connectorConfig,
                    new KingbaseESChangeEventSourceFactory(
                            connectorConfig,
                            snapshotter,
                            jdbcConnection,
                            errorHandler,
    ...
    public ReplicationConnection createReplicationConnection(KingbaseESTaskContext taskContext, boolean doSnapshot, int maxRetries, Duration retryDelay)
    ...
    public KingbaseESTaskContext getTaskContext() {
        return taskContext;
    }
}
