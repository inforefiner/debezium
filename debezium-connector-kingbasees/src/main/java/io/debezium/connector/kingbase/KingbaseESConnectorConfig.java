...
import io.debezium.connector.kingbase.connection.MessageDecoder;
import io.debezium.connector.kingbase.connection.MessageDecoderContext;
import io.debezium.connector.kingbase.connection.ReplicationConnection;
import io.debezium.connector.kingbase.connection.pgoutput.PgOutputMessageDecoder;
import io.debezium.connector.kingbase.connection.pgproto.PgProtoMessageDecoder;
import io.debezium.connector.kingbase.connection.wal2json.NonStreamingWal2JsonMessageDecoder;
import io.debezium.connector.kingbase.connection.wal2json.StreamingWal2JsonMessageDecoder;
import io.debezium.connector.kingbase.snapshot.AlwaysSnapshotter;
import io.debezium.connector.kingbase.snapshot.InitialOnlySnapshotter;
import io.debezium.connector.kingbase.snapshot.InitialSnapshotter;
import io.debezium.connector.kingbase.snapshot.NeverSnapshotter;
...
/**
 * The configuration properties for the {@link KingbaseESConnector}
 *
 * @author Horia Chiorean
 */
public class KingbaseESConnectorConfig extends RelationalDatabaseConnectorConfig {

    private static final Logger LOGGER = LoggerFactory.getLogger(KingbaseESConnectorConfig.class);
    ...
    protected static final int DEFAULT_PORT = 5_4321;
    ...
    public static final Field SLOT_NAME = Field.create("slot.name")
            ...
            .withValidation(KingbaseESConnectorConfig::validateReplicationSlotName)
    ...
    public static final Field TRUNCATE_HANDLING_MODE = Field.create("truncate.handling.mode")
            ...
            .withWidth(Width.MEDIUM)
            .withImportance(Importance.MEDIUM)
            .withValidation(KingbaseESConnectorConfig::validateTruncateHandlingMode)
    ...
    public KingbaseESConnectorConfig(Configuration config) {
        super(
                config,
        ...
        this.truncateHandlingMode = TruncateHandlingMode.parse(config.getString(KingbaseESConnectorConfig.TRUNCATE_HANDLING_MODE));
        String hstoreHandlingModeStr = config.getString(KingbaseESConnectorConfig.HSTORE_HANDLING_MODE);
        this.hStoreHandlingMode = HStoreHandlingMode.parse(hstoreHandlingModeStr);
        this.intervalHandlingMode = IntervalHandlingMode.parse(config.getString(KingbaseESConnectorConfig.INTERVAL_HANDLING_MODE));
    ...
    protected Duration statusUpdateInterval() {
        return Duration.ofMillis(getConfig().getLong(KingbaseESConnectorConfig.STATUS_UPDATE_INTERVAL_MS));
    }
    ...
    protected Duration xminFetchInterval() {
        return Duration.ofMillis(getConfig().getLong(KingbaseESConnectorConfig.XMIN_FETCH_INTERVAL));
    }
    ...
    protected SourceInfoStructMaker<? extends AbstractSourceInfo> getSourceInfoStructMaker(Version version) {
        switch (version) {
            ...
            default:
                return new KingbaseESSourceInfoStructMaker(Module.name(), Module.version(), this);
        }
    }
