 ...
 */
public class KingbaseESChangeEventSourceCoordinator extends ChangeEventSourceCoordinator<KingbaseESPartition, KingbaseESOffsetContext> {

    private static final Logger LOGGER = LoggerFactory.getLogger(KingbaseESChangeEventSourceCoordinator.class);

    private final Snapshotter snapshotter;
    private final SlotState slotInfo;

    public KingbaseESChangeEventSourceCoordinator(Offsets<KingbaseESPartition, KingbaseESOffsetContext> previousOffsets,
                                                  ErrorHandler errorHandler,
                                                  Class<? extends SourceConnector> connectorType,
                                                  CommonConnectorConfig connectorConfig,
                                                  KingbaseESChangeEventSourceFactory changeEventSourceFactory,
    ...
    protected CatchUpStreamingResult executeCatchUpStreaming(ChangeEventSourceContext context,
                                                             SnapshotChangeEventSource<KingbaseESPartition, KingbaseESOffsetContext> snapshotSource,
                                                             KingbaseESPartition partition,
                                                             KingbaseESOffsetContext previousOffset)
            throws InterruptedException {
        if (previousOffset != null && !snapshotter.shouldStreamEventsStartingFromSnapshot() && slotInfo != null) {
            try {
                setSnapshotStartLsn((KingbaseESSnapshotChangeEventSource) snapshotSource,
                        previousOffset);
            }
    ...
    private void setSnapshotStartLsn(KingbaseESSnapshotChangeEventSource snapshotSource,
