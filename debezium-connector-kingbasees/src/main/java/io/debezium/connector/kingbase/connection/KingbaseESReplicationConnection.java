 ...
 */
public class KingbaseESReplicationConnection extends JdbcConnection implements ReplicationConnection {

    private static Logger LOGGER = LoggerFactory.getLogger(KingbaseESReplicationConnection.class);

    private final String slotName;
    private final String publicationName;
    private final RelationalTableFilters tableFilter;
    private final KingbaseESConnectorConfig.AutoCreateMode publicationAutocreateMode;
    private final KingbaseESConnectorConfig.LogicalDecoder plugin;
    private final boolean dropSlotOnClose;
    private final KingbaseESConnectorConfig originalConfig;
     ...
     */
    private KingbaseESReplicationConnection(KingbaseESConnectorConfig config,
                                            String slotName,
                                            String publicationName,
                                            RelationalTableFilters tableFilter,
                                            KingbaseESConnectorConfig.AutoCreateMode publicationAutocreateMode,
                                            KingbaseESConnectorConfig.LogicalDecoder plugin,
                                            boolean dropSlotOnClose,
                                            boolean doSnapshot,
                                            Duration statusUpdateInterval,
                                            TypeRegistry typeRegistry,
                                            Properties streamParams,
                                            KingbaseESSchema schema) {
        super(config.getJdbcConfig(), KingbaseESConnection.FACTORY, null, KingbaseESReplicationConnection::defaultSettings, "\"", "\"");
    ...
    private ServerInfo.ReplicationSlot getSlotInfo() throws SQLException, InterruptedException {
        try (KingbaseESConnection connection = new KingbaseESConnection(originalConfig.getJdbcConfig())) {
    ...
    protected void initPublication() {
        ...
        if (KingbaseESConnectorConfig.LogicalDecoder.PGOUTPUT.equals(plugin)) {
    ...
    public ReplicationStream startStreaming(Lsn offset, WalPositionLocator walPosition) throws SQLException, InterruptedException {
        ...
        int maxRetries = config().getInteger(KingbaseESConnectorConfig.MAX_RETRIES);
        int delay = config().getInteger(KingbaseESConnectorConfig.RETRY_DELAY_MS);
        int tryCount = 0;
        while (true) {
            try {
    ...
    private KBReplicationStream startPgReplicationStream(final Lsn lsn, Function<ChainedLogicalStreamBuilder, ChainedLogicalStreamBuilder> configurator)
            throws SQLException {
        assert lsn != null;
    ...
    protected static void defaultSettings(Configuration.Builder builder) {
        ...
        KingbaseESConnection.defaultSettings(builder);
    ...
    protected static class ReplicationConnectionBuilder implements Builder {
        ...
        private KingbaseESConnectorConfig.AutoCreateMode publicationAutocreateMode = KingbaseESConnectorConfig.AutoCreateMode.ALL_TABLES;
        private KingbaseESConnectorConfig.LogicalDecoder plugin = KingbaseESConnectorConfig.LogicalDecoder.DECODERBUFS;
        private KingbaseESConnectorConfig.TruncateHandlingMode truncateHandlingMode;
        private boolean dropSlotOnClose = DEFAULT_DROP_SLOT_ON_CLOSE;
        private Duration statusUpdateIntervalVal;
        private boolean doSnapshot;
        private TypeRegistry typeRegistry;
        private KingbaseESSchema schema;
        private Properties slotStreamParams = new Properties();

        protected ReplicationConnectionBuilder(KingbaseESConnectorConfig config) {
            assert config != null;
            this.config = config;
        }
        ...
        @Override
        public Builder withPublicationAutocreateMode(KingbaseESConnectorConfig.AutoCreateMode publicationAutocreateMode) {
            assert publicationName != null;
            this.publicationAutocreateMode = publicationAutocreateMode;
            return this;
        }
        ...
        @Override
        public ReplicationConnectionBuilder withPlugin(final KingbaseESConnectorConfig.LogicalDecoder plugin) {
            assert plugin != null;
            this.plugin = plugin;
            return this;
        }
        ...
        @Override
        public Builder withTruncateHandlingMode(KingbaseESConnectorConfig.TruncateHandlingMode truncateHandlingMode) {
        ...
        public ReplicationConnection build() {
            ...
            return new KingbaseESReplicationConnection(config, slotName, publicationName, tableFilter, publicationAutocreateMode, plugin,
        ...
        @Override
        public Builder withSchema(KingbaseESSchema schema) {
            this.schema = schema;
            return this;
        }
    }
