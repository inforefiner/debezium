 ...
 */
package io.debezium.connector.kingbase.connection.pgoutput;
...
import io.debezium.connector.kingbase.connection.ReplicationMessage.Column;
import io.debezium.connector.kingbase.connection.ReplicationMessage.NoopMessage;
import io.debezium.connector.kingbase.connection.ReplicationMessage.Operation;
import io.debezium.connector.kingbase.connection.ReplicationStream.ReplicationMessageProcessor;
...
public class PgOutputMessageDecoder extends AbstractMessageDecoder {
    ...
    public PgOutputMessageDecoder(MessageDecoderContext decoderContext) {
        ...
        this.connection = new KingbaseESConnection(decoderContext.getConfig().getJdbcConfig(), decoderContext.getSchema().getTypeRegistry());
    }
    ...
    public void processNotEmptyMessage(ByteBuffer buffer, ReplicationMessageProcessor processor, TypeRegistry typeRegistry) throws SQLException, InterruptedException {
        if (LOGGER.isTraceEnabled()) {
            if (!buffer.hasArray()) {
                throw new IllegalStateException("Invalid buffer received from KB server during streaming replication");
            }
        ...
        switch (messageType) {
            ...
            case DELETE:
                ...
                break;
            case TRUNCATE:
                if (decoderContext.getConfig().truncateHandlingMode() == KingbaseESConnectorConfig.TruncateHandlingMode.INCLUDE) {
    ...
    private void handleRelationMessage(ByteBuffer buffer, TypeRegistry typeRegistry) throws SQLException {
        ...
        for (short i = 0; i < columnCount; ++i) {
            ...
            final KingbaseESType postgresType = typeRegistry.get(columnType);
    ...
    private List<io.debezium.relational.Column> getTableColumnsFromDatabase(KingbaseESConnection connection, DatabaseMetaData databaseMetadata, TableId tableId)
            throws SQLException {
    ...
    private static List<Column> resolveColumnsFromStreamTupleData(ByteBuffer buffer, TypeRegistry typeRegistry, Table table) {
        ...
        for (short i = 0; i < numberOfColumns; ++i) {
            ...
            final KingbaseESType columnType = typeRegistry.get(typeName);
