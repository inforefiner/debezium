...
class PgProtoReplicationMessage implements ReplicationMessage {
    ...
    private List<Column> transform(List<PgProto.DatumMessage> messageList, List<PgProto.TypeInfo> typeInfoList) {
        return IntStream.range(0, messageList.size())
                .mapToObj(index -> {
                    ...
                    final KingbaseESType type = typeRegistry.get((int) datum.getColumnType());
    ...
    public Object getValue(String columnName, KingbaseESType type, String fullType, PgProto.DatumMessage datumMessage, final PgConnectionSupplier connection,
