...
/**
 * Component that records the schema information for the {@link KingbaseESConnector}. The schema information contains
 * the {@link Tables table definitions} and the Kafka Connect {@link #schemaFor(TableId) Schema}s for each table, where the
 * {@link Schema} excludes any columns that have been {@link KingbaseESConnectorConfig#COLUMN_EXCLUDE_LIST specified} in the
 * configuration.
 *
 * @author Horia Chiorean
 */
@NotThreadSafe
public class KingbaseESSchema extends RelationalDatabaseSchema {

    protected final static String PUBLIC_SCHEMA_NAME = "public";
    private final static Logger LOGGER = LoggerFactory.getLogger(KingbaseESSchema.class);
    ...
    /**
     * Create a schema component given the supplied {@link KingbaseESConnectorConfig Postgres connector configuration}.
     *
     * @param config the connector configuration, which is presumed to be valid
     */
    protected KingbaseESSchema(KingbaseESConnectorConfig config, TypeRegistry typeRegistry,
                               TopicSelector<TableId> topicSelector, KingbaseESValueConverter valueConverter) {
    ...
    private static TableSchemaBuilder getTableSchemaBuilder(KingbaseESConnectorConfig config, KingbaseESValueConverter valueConverter) {
     ...
     */
    protected KingbaseESSchema refresh(KingbaseESConnection connection, boolean printReplicaIdentityInfo) throws SQLException {
    ...
    private void printReplicaIdentityInfo(KingbaseESConnection connection, TableId tableId) {
        try {
     ...
     */
    protected void refresh(KingbaseESConnection connection, TableId tableId, boolean refreshToastableColumns) throws SQLException {
    ...
    private void refreshToastableColumnsMap(KingbaseESConnection connection, TableId tableId) {
