...
/**
 * The context of a {@link KingbaseESConnectorTask}. This deals with most of the brunt of reading various configuration options
 * and creating other objects with these various options.
 *
 * @author Horia Chiorean (hchiorea@redhat.com)
 */
@ThreadSafe
public class KingbaseESTaskContext extends CdcSourceTaskContext {

    protected final static Logger LOGGER = LoggerFactory.getLogger(KingbaseESTaskContext.class);
    ...
    protected KingbaseESTaskContext(KingbaseESConnectorConfig config, KingbaseESSchema schema, TopicSelector<TableId> topicSelector) {
    ...
    protected KingbaseESSchema schema() {
        return schema;
    }

    protected KingbaseESConnectorConfig config() {
        return config;
    }

    protected void refreshSchema(KingbaseESConnection connection, boolean printReplicaIdentityInfo) throws SQLException {
        schema.refresh(connection, printReplicaIdentityInfo);
    }

    Long getSlotXmin(KingbaseESConnection connection) throws SQLException {
    ...
    private SlotState getCurrentSlotState(KingbaseESConnection connection) throws SQLException {
    ...
    protected ReplicationConnection createReplicationConnection(boolean doSnapshot) throws SQLException {
        ...
        if (dropSlotOnStop) {
            LOGGER.warn(
                    ...
                    KingbaseESConnectorConfig.DROP_SLOT_ON_STOP.name());
        }
    ...
    KingbaseESConnectorConfig getConfig() {
        return config;
    }
}
