...
public class TypeRegistry {
    ...
    private final Map<String, KingbaseESType> nameToType = new HashMap<>();
    private final Map<Integer, KingbaseESType> oidToType = new HashMap<>();
    ...
    public TypeRegistry(KingbaseESConnection connection) {
        try {
    ...
    private void addType(KingbaseESType type) {
     ...
     */
    public KingbaseESType get(int oid) {
        KingbaseESType r = oidToType.get(oid);
        if (r == null) {
            r = resolveUnknownType(oid);
            if (r == null) {
                LOGGER.warn("Unknown OID {} requested", oid);
                r = KingbaseESType.UNKNOWN;
            }
        }
        return r;
    }
     ...
     */
    public KingbaseESType get(String name) {
        switch (name) {
            case "serial":
                name = "int4";
                break;
        ...
        KingbaseESType r = nameToType.get(name);
        if (r == null) {
    ...
    private void prime() throws SQLException {
        try (final Statement statement = connection.createStatement();
            ...
            final List<KingbaseESType.Builder> delayResolvedBuilders = new ArrayList<>();
            while (rs.next()) {
                KingbaseESType.Builder builder = createTypeBuilderFromResultSet(rs);
            ...
            for (KingbaseESType.Builder builder : delayResolvedBuilders) {
                addType(builder.build());
            }
        }
    }

    private KingbaseESType.Builder createTypeBuilderFromResultSet(ResultSet rs) throws SQLException {
        ...
        KingbaseESType.Builder builder = new KingbaseESType.Builder(
                this,
                typeName,
                oid,
    ...
    private KingbaseESType resolveUnknownType(String name) {
        try {
    ...
    private KingbaseESType resolveUnknownType(int lookupOid) {
        try {
    ...
    private KingbaseESType loadType(PreparedStatement statement) throws SQLException {
        try (final ResultSet rs = statement.executeQuery()) {
            while (rs.next()) {
                KingbaseESType result = createTypeBuilderFromResultSet(rs).build();
                addType(result);
                return result;
            }
        }
